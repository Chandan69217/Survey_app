...
class _RaiseQueryScreenState extends State<RaiseQueryScreen> {
  ...
  void _fetchQueries()async{
    ...
    final response = await _getAllRaisedQuery(page: _currentIndex.toString());
    if(response == null){
      setState(() {
        _isLoading = false;
      });
  ...
  Future<Map<String,dynamic>?> _getAllRaisedQuery({required String page})async{
    try{
      final url = Uri.https(Urls.baseUrl,Urls.public_query_list,{'page' : _currentIndex.toString()});
  ...
  Future<Map<String, dynamic>?> _registerQuery({
    ...
    try{
      final url = Uri.https(Urls.baseUrl,Urls.public_query_entry );
      final headers = {
        'Content-Type': 'application/json',
        'Client-source' : 'mobile'
      };
